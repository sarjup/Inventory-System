...
class Purchase(db.Model):
    ...
    s_id = db.Column(db.Integer, ForeignKey("tbl_supplier.id"), nullable =False)
    p_id = db.Column(db.Integer, ForeignKey("tbl_product.id"), nullable = False)
    pro_qty = db.Column(db.Integer, nullable = False)
    pro_price = db.Column(db.Numeric(8,2), nullable =False)

    __table_args__=(
        CheckConstraint(pro_qty>= 1, name = "check_quantity_entered"),{}
    )

    def __init__(self, bill_no, date, s_id, p_id, pro_qty, pro_price):
...
class PurchaseSchema(ma.Schema):
    class Meta:
        fields = ('id','p_name','p_qty','c_price','s_price','s_name','bill_no','date')
